...
from tensorflow.keras.layers import Input, concatenate, MaxPooling1D, Conv1D, Dense, Dropout, Embedding, Flatten
...
def define_model_CNN_word_word(length, vocab_size, embedding_matrix, word_index,EMBEDDING_DIM, MAX_SEQUENCE_LENGTH):
    in_1 = Input(shape=(MAX_SEQUENCE_LENGTH), dtype='float32', name='fixed_input')
    in_2 = Input(shape=(MAX_SEQUENCE_LENGTH), dtype='float32', name='dynam_input')

    em_1 = Embedding(len(word_index) + 1, EMBEDDING_DIM,
                     tf.initializers.constant(embedding_matrix), trainable=True)(in_1)
    em_2 = Embedding(len(word_index) + 1, EMBEDDING_DIM,
                     tf.initializers.constant(embedding_matrix), trainable=False)(in_2)

    x = concatenate([em_1, em_2], axis=-1)
    x = Conv1D(64, 5, activation="relu")(x)


    x = MaxPooling1D(MAX_SEQUENCE_LENGTH - 5 + 1)(x)
    x = Flatten()(x)
    x = Dense(256, activation="relu")(x)
    x = Dropout(0.5)(x)
    x = Dense(int(sys.argv[1]), activation="softmax")(x)
    # metrics = Metrics()

    model = Model(inputs=[in_1, in_2], outputs = x)
...
embedding_matrix = load_model('glove.6B.300d.txt')
